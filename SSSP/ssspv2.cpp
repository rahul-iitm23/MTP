...
using namespace std;
/*
int findMin(int *sssp, bool *visited, int n)
{
	int mn = INT_MAX;
	int mn_index = -1;
	for(int i=0;i<n; i++)
	{
		if(!visited[i] && sssp[i]< mn)
		{
			mn = sssp[i];
			mn_index = i;
		}
...
int main(int argc, char *arg[])
{
	...
	int *vertex_index = G.indexofNodes;
	int *vertex_neighbour = G.edgeList;
...
// copy data from host to device
	...
	status = clEnqueueWriteBuffer(command_queue, neighbour_device, CL_TRUE,0, (e)*sizeof(int), vertex_neighbour,0,NULL, NULL);
...
// create OpenCl kernel from the program
	cl_kernel kernel = clCreateKernel(program, "ssspKernelPush", &status);
...
do{
	*changed = 0;
	...
	{
		cout<<"kernel execution unsuccessful "<< status<<endl;
	}
...
cout<<"Time spent in initialization = "<<timeInInitialization<<" sec."<<endl;
cout<<"Time spent in kernel = "<<timeInKernel<<" sec"<<endl;
cout<<"Total time for SSSP: "<< timeInKernel + timeInInitialization<<endl;
