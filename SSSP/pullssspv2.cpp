...
// Memory allocation on device
	cl_mem d_revIndexCSR = clCreateBuffer(context, CL_MEM_READ_ONLY, (n+1)*sizeof(int), NULL, &status);
	cl_mem 	d_CSC = clCreateBuffer(context, CL_MEM_READ_ONLY, (e)*sizeof(int), NULL, &status);
	cl_mem d_indexCSR = clCreateBuffer(context, CL_MEM_READ_ONLY, (n+1)*sizeof(int), NULL, &status);
	cl_mem d_CSR = clCreateBuffer(context, CL_MEM_READ_ONLY, e*sizeof(int), NULL, &status);
	cl_mem d_sssp = clCreateBuffer(context, CL_MEM_READ_WRITE, n*sizeof(int), NULL, &status);
	cl_mem d_changed = clCreateBuffer(context, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR,sizeof(int), NULL, &status);
	//cl_mem d_wlSize = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, &status);
// copy data from host to device
	status = clEnqueueWriteBuffer(command_queue, d_revIndexCSR, CL_TRUE, 0, (n+1)*sizeof(int), revIndexCSR, 0, NULL, NULL);
	status = clEnqueueWriteBuffer(command_queue, d_CSC, CL_TRUE,0, (e)*sizeof(int), CSC,0,NULL, NULL);
	status = clEnqueueWriteBuffer(command_queue, d_indexCSR, CL_TRUE, 0, (n+1)*sizeof(int), indexCSR, 0, NULL, NULL);
	status = clEnqueueWriteBuffer(command_queue, d_CSR, CL_TRUE, 0, e*sizeof(int), CSR, 0, NULL, NULL);
...
status = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&d_revIndexCSR);
status = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&d_CSC);
status = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&d_indexCSR);
status = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*)&d_CSR);
status  = clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *)&d_sssp);
status = clSetKernelArg(kernel, 5, sizeof(cl_mem), (void *)&d_changed);
status = clSetKernelArg(kernel, 6, sizeof(int), (void *)&n);


	//kernel arguments for initialization kernel
status = clSetKernelArg(init, 0, sizeof(int), (void *)&source);
status = clSetKernelArg(init, 1, sizeof(cl_mem), (void *)&d_sssp);
...
changed = (int *)clEnqueueMapBuffer(command_queue, d_changed, CL_TRUE, CL_MAP_READ|CL_MAP_WRITE, 0,sizeof(int),0,NULL,NULL,&status);
...
clEnqueueUnmapMemObject(command_queue, d_changed, changed, 0, NULL, NULL);
...
// copy result to host
status = clEnqueueReadBuffer(command_queue, d_sssp , CL_TRUE, 0, n*sizeof(int), sssp, 0, NULL, &event2);
...
status = clReleaseMemObject(d_revIndexCSR);
status = clReleaseMemObject(d_indexCSR);
status = clReleaseMemObject(d_CSC);
status = clReleaseMemObject(d_CSR);
status = clReleaseMemObject(d_sssp);
status = clReleaseMemObject(d_changed);
