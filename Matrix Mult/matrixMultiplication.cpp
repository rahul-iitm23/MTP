...
int main(int argc, char *arg[])
{
	...
	cl_mem d_C = clCreateBuffer(context, CL_MEM_WRITE_ONLY,p*r*sizeof(int), NULL, &status);
	...
	// event variable for profiling 
	cl_event event;
	// Execute the openCL kernel
	const size_t global_size[2] = {(size_t)p,(size_t)r};
	const size_t local_size[2] = {16, 16};

	status = clEnqueueNDRangeKernel(command_queue, kernel , 2, NULL, global_size, local_size, 0, NULL, &event);
	...
	cout<<"Total time spent in matrix multiplication kernel= "<<(double(end-start))/x<<" seconds"<<endl;
